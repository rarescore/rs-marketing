...
export default async function ConsultationPage({ searchParams }: ConsultationPageProps) {
  ...
  return (
    <main className="re-main re-contact" id="real-estate-main">
      <div className="re-contact__intro"><p className="re-kicker">{owner ? "Showroom consultation" : "Atelier North consultation · Demonstration"}</p><h1>{owner ? "See this system with your market and brand." : "Begin with the decision in front of you."}</h1><p>{owner ? "This owner-facing path is separate from the fictional brokerage. Use it to preview how the website, tools, qualification, and follow-up system could fit a real practice." : "Choose the context, preferred timing, and only the details useful for a first conversation. This demonstration validates but does not send or retain the request."}</p><dl><div><dt>What happens here</dt><dd>Server validation and a demo receipt</dd></div><div><dt>What does not happen</dt><dd>No brokerage contact, CRM record, email, or data retention</dd></div></dl></div>
