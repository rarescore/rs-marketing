...
const choiceLabels: Record<Choice, string> = { yes: "Yes", no: "No", "not-sure": "I’m not sure" };
...
export function CallbackIntake({ onlineReady }: { onlineReady: boolean }) {
  ...
  return <div className="il-callback">
    ...
    <div className="il-callback__stage">
      ...
      {step === 3 && <><p className="il-eyebrow">Injury</p><h3>Tell us only what you know now.</h3><fieldset><legend>Were you injured?</legend><div className="il-callback__choices">{(["yes","no","not-sure"] as Choice[]).map((value) => <button type="button" aria-pressed={injured === value} key={value} onClick={() => setInjured(value)}>{choiceLabels[value]}</button>)}</div></fieldset><fieldset><legend>Have you received medical attention?</legend><div className="il-callback__choices">{(["yes","no","not-sure"] as Choice[]).map((value) => <button type="button" aria-pressed={medical === value} key={value} onClick={() => setMedical(value)}>{choiceLabels[value]}</button>)}</div></fieldset><button className="il-button il-button--oxblood" type="button" onClick={advance}>Continue</button></>}
