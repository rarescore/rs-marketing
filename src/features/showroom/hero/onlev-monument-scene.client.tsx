...
function AnimatedPart({
  ...
  useFrame((_, delta) => {
    ...
    const p = phase(useOnlevOpening.getState().progress, 0.04, 0.72);
...
function Monument() {
  ...
  useFrame(({ pointer }, delta) => {
    ...
    const p = useOnlevOpening.getState().progress;
...
function SevenLayers() {
  ...
  useFrame((_, delta) => {
    if (!group.current) return;
    const p = phase(useOnlevOpening.getState().progress, 0.18, 0.82);
