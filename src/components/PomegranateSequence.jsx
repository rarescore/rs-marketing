  ...
  useEffect(() => {
    ...
    const finish = () => {
      ...
      window.setTimeout(() => {
        ...
        if (next) window.scrollTo({ top: next.offsetTop, behavior: 'auto' });
      }, 300);
    };
    ...
    const render = (time) => {
      ...
      // Keep the camera move on the exact same clock as the falling seeds.
      const zoomStart = 0.68;
      const zoomProgress = clamp((progress - zoomStart) / (1 - zoomStart), 0, 1);
      const zoomEase = 1 - Math.pow(1 - zoomProgress, 3);
      canvas.style.transform = `scale(${1 + zoomEase * 0.22})`;
      canvas.style.filter = `brightness(${1 - zoomEase * 0.12})`;
      sectionRef.current?.style.setProperty('--intro-progress', String(progress));
      sectionRef.current?.style.setProperty('--zoom-progress', String(zoomEase));
